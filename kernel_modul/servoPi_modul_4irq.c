...
void free_irq_fn(void){
		...
		free_irq((unsigned int)irc3_irq_num, &irc0);
		free_irq((unsigned int)irc2_irq_num, &irc0);
		free_irq((unsigned int)irc4_irq_num, &irc0);
}
...
static int servoPi_init(void) {
	...
	if(request_irq((unsigned int)irc1_irq_num, irc_irq_handlerAS, IRQF_TRIGGER_FALLING, "irc1_irqAS", &irc0) != 0){
		printk(KERN_ERR "failed request IRQ GPIO 2\n");
		free_fn();
		return (-1);
	}
	if(request_irq((unsigned int)irc3_irq_num, irc_irq_handlerAN, IRQF_TRIGGER_RISING, "irc3_irqAN", &irc0) != 0){
		printk(KERN_ERR "failed request IRQ GPIO 23\n");
		free_fn();
		free_irq((unsigned int)irc1_irq_num, &irc0);
		return (-1);
	}
	if(request_irq((unsigned int)irc2_irq_num, irc_irq_handlerBS, IRQF_TRIGGER_FALLING, "irc2_irqBN", &irc0) != 0){
	...
	if(request_irq((unsigned int)irc4_irq_num, irc_irq_handlerBN, IRQF_TRIGGER_RISING, "irc4_irqBS", &irc0) != 0){
		printk(KERN_ERR "failed request IRQ GPIO 3\n");
		free_fn();
		free_irq((unsigned int)irc1_irq_num, &irc0);
		free_irq((unsigned int)irc3_irq_num, &irc0);
